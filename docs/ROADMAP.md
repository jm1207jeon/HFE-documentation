...
- [x] 캔버스: 팔레트 더블클릭 배치, 선택, 드래그 이동, 방향키 미세 이동, 스냅(1mm/8px),
      정렬 기준 격자(종이 10mm·화면 40px), 줌(확대/축소/100%/화면 맞춤 버튼 + Ctrl+±/0/F)
      — 다중 선택·리사이즈 핸들은 v2
...
- [x] 최대 3개 병렬 비교 창(총점 카드·카테고리 점수 표·Finding diff) — Variant 복제(variantOf
      자동 연결)는 v2
- [x] 총점 막대(배치안별 누적 막대) + 카테고리 점수 표
- [ ] 카테고리 레이더 차트(자체 Canvas 렌더링), 미니 캔버스 썸네일 — 데이터는
...
- [x] 앱 자체 HFE 준수 테스트(`AppSelfComplianceTests`) — 자신이 요구하는 기준(최소 폰트 12px,
      ...
      앱 XAML이 지키는지 검사
- [x] WPF 실행 스모크 테스트(`tests/HfeLayoutSim.App.Tests`) — 실제 Window를 띄워 팔레트 전량 배치,
      Undo/Redo 왕복, 저장·재열기 동일성, 손상 파일 방어, 평가·무효화, HTML 내보내기, 비교,
      캔버스 경계 클램프를 검증하고 **WPF 바인딩 오류 0건**을 단언 (Windows 전용 — 솔루션 밖,
      CI에서 별도 실행)
